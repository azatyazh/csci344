...
class AccessTokenEndpoint(Resource):
    ...
    def post(self):
        ...
        '''
        +1. We need to query the database for a user matching the username
        +    - If user doesn't exist, throw an error and terminate early.
        2. Chec, that the password matches:
            - If password doesn't match, throw an error and terminate early. 
        3. If we get here, we know the user is legit. Give them access token and refresh token.
        '''
        ...
        if the_user is None: 
            return Response(
                json.dumps({"message": "User not found in database"}),
                mimetype="application/json",
                status=401,
            )
        

        # 2. Check that the passwords match by using the built-in function:
        if the_user.check_password(password) is False:
            return Response(
                json.dumps({"message": "Password incorrect"}),
                mimetype="application/json",
                status=401,
            )
        

        # 3. Create access and refresh tokens, which will embed the logged
            # in user's user_id(the_user.id)
